...
def plotingfinals():
    ...
    def cnot ():
        ...
        if choice in {'Y','YES','yes','Yes','y'}:
            ...
            if specific_prn in grouped.groups:
                group = grouped.get_group(specific_prn)
                fig = px.line(group, x='24-hour Time', y='C/N0', title=f'C/NO values for PRN {specific_prn}')
                fig.update_layout(
                    xaxis_title='Time (24-hour format)',
                    yaxis_title='C/N0)',
                    xaxis_tickformat='%H:%M:%S'
            ...
            # Update layout
            fig.update_layout(
                title='C/NO values for all PRNs',
                xaxis_title='Time (24-hour format)',
                yaxis_title='C/N0',
                xaxis_tickformat='%H:%M:%S'
            )
    ...
    def elev():
        ...
        if choice in {'Y','YES','yes','Yes','y'}:
            ...
            # Update layout
            fig.update_layout(
                title='Elevation values for all PRNs',
                xaxis_title='Time (24-hour format)',
                yaxis_title='Elevation',
                xaxis_tickformat='%H:%M:%S'
            )
    ...
    def elevdu():
        ...
        if choice in {'Y','YES','yes','Yes','y'}:
            ...
            if specific_prn in grouped.groups:
                ...
                # Add C/N0 values
                fig.add_trace(go.Scatter(x=group['Time'], y=group['Elevation'], mode='lines', name='Elevation', yaxis='y2'))
                
                # Update layout to have two y-axes
                fig.update_layout(
                    title=f'Elevation and d/u values for PRN {specific_prn}',
                    xaxis_title='Time (24-hour format)',
                    xaxis_tickformat='%H:%M:%S',
                    yaxis=dict(
                        title='d/u (dB)',
    ...
    def residual():
        ...
        if choice in {'Y','YES','yes','Yes','y'}:
            ...
            if specific_prn in grouped.groups:
                group = grouped.get_group(specific_prn)
                fig = px.line(group, x='24-hour Time', y='Residual', title=f'Resiudals values for PRN {specific_prn}')
                fig.update_layout(
                    xaxis_title='Time (24-hour format)',
                    yaxis_title='d/u (dB)',
                    xaxis_tickformat='%H:%M:%S'
            ...
            # Update layout
            fig.update_layout(
                title='Residuals values for all PRNs',
                xaxis_title='Time (24-hour format)',
